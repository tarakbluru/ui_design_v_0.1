    ...
    layout = [
        [sg.TabGroup([
            [sg.Tab('NIFTY', [
                ...
                [sg.Combo(['NIFTY-I', 'NIFTY_INDEX'], key='-NIFTY-OPTION-', default_value='NIFTY-I', enable_events=True, size=(20, 1)), sg.Combo(['MIS', 'NRML'], key='-NIFTY-MODE-', default_value='MIS', enable_events=True, size=(20, 1)), sg.CalendarButton('Pick a Date', target='-NIFTY-DATE-', format='%d/%m/%Y', pad=(10, 10)), sg.InputText('', key='-NIFTY-DATE-', size=(20, 1), enable_events=True), sg.Text('Or', size=3), sg.Combo(['WEEK-I', 'WEEK-II', 'MONTH-I'], key='-EXPIRY-OPTION-', default_value='WEEK-I', enable_events=True, size=(20, 1))],
                [sg.Text('Moneyness (Ix Based):', size=20), sg.Text('~ Premium:',size=20), sg.Text('Select Broker:',size=20), sg.Text('Target Ratios:', size=20)],
                [sg.Combo(['ATM-5', 'ATM-4', 'ATM-3', 'ATM-2', 'ATM-1', 'ATM', 'ATM+1', 'ATM+2', 'ATM+3', 'ATM+4', 'ATM+5'], key='-NIFTY-ATM-', default_value='ATM', size=(20, 1), enable_events=True),
                sg.InputText('', key='-NIFTY-PREMIUM-',size=(20,1)),
                sg.Combo(['FINVASIA', 'ZERODHA'], key='-BROKER-OPTION-', default_value='FINVASIA', enable_events=True, size=(25, 1)), sg.Combo(['0:0:3', '0:3:0','3:0:0', '1:1:1', '1:2:0', '2:1:0', '0:1:2', '0:2:1'], key='-NIFTY-TARGET_RATIO-', default_value='1:1:1', enable_events=True, size=(10, 1))
                ],
                
                # Section 2: Entry / Target / SL Levels
                [sg.Text('Levels', font=('Helvetica', 14), relief=sg.RELIEF_RIDGE, size=(80,1))],

                [sg.Text('NIFTY Long Entry Level:',size=25), sg.Text('NIFTY Short Entry Level:',size=(25,1), justification='left'), sg.Text('NIFTY Alert Level:', size=25)], 
                [sg.InputText('', key='-NIFTY-BUY-LEVEL-', size=25), sg.InputText('', key='-NIFTY-SHORT-LEVEL-', size=25), sg.InputText('', key='-NIFTY-ALERT-LEVEL-',size=25)],

                [sg.Text('Long T1 Level:',size=25), sg.Text('Long T2 Level:',size=25), sg.Text('Long T3 Level:',size=25), sg.Text('Long SL Level:',size=25)], 
                [sg.InputText('', key='-NIFTY-LONG-TARGET1-LEVEL-',size=25), sg.InputText('', key='-NIFTY-LONG-TARGET2-LEVEL-',size=25), sg.InputText('', key='-NIFTY-LONG-TARGET3-LEVEL-',size=25), sg.InputText('', key='-NIFTY-LONG-SL-LEVEL-',size=25)],

                [sg.Text('Short T1 Level:',size=25), sg.Text('Short T2 Level:',size=25), sg.Text('Short T3 Level:',size=25), sg.Text('Short SL Level:',size=25)], 
                [sg.InputText('', key='-NIFTY-SHORT-TARGET1-LEVEL-',size=25), sg.InputText('', key='-NIFTY-SHORT-TARGET2-LEVEL-',size=25),sg.InputText('', key='-NIFTY-SHORT-TARGET1-LEVEL-',size=25), sg.InputText('', key='-NIFTY-SHORT-SL-LEVEL-',size=25)],

                # Section 3: Trading Info
                [sg.Text('Position Sizing', font=('Helvetica', 14), relief=sg.RELIEF_RIDGE, size=(80,1))],
                [sg.Text('Qty(in Lots):',size=20), sg.Text('Qty(in Amount):', size=20)],
                [sg.InputText('', key='-NIFTY-QTY-LOTS-', size=20), sg.InputText('', key='-NIFTY-QTY-AMOUNT',size=20)],
                [sg.Text('', size=(80, 1))],
                [sg.Button('NIFTY CE BUY  \u2191', button_color=('white', 'green'), size=(30,2)),  sg.Text('',size=10), sg.Button('NIFTY CE SELL \u2193', button_color=('white', 'red'),size=(30,2))],  # CE buttons with up and down arrows
                [sg.Text('', size=(80, 1))],
                [sg.Button('NIFTY PE SELL \u2191', button_color=('white', 'green'), size=(30,2)),  sg.Text('',size=10), sg.Button('NIFTY PE BUY  \u2193', button_color=('white', 'red'),size=(30,2)) ],  # PE buttons with up and down arrows
                [sg.Text('', size=(80, 1))]
            ])],
            [sg.Tab('Bank Nifty', [
        ...
        [sg.Text('', size=(80, 1))],
        [sg.Text('Overall Per Day Max PNL', font=('Helvetica', 14), relief=sg.RELIEF_RIDGE, size=(80,1))],
        [sg.Text('Max Loss:', size=20), sg.Text('Max Profit:', size=20)],
        [sg.InputText('', key='-NIFTY-MAX-LOSS-',size=20), sg.InputText('', key='-NIFTY-MAX-PROFIT-',size=20)],
        [sg.Text('', size=(80, 1))],
        [sg.Text('', size=(80, 1))],
